...
function createNaporiForm() {
  ...
  // ---- セクション5：イベント（背景情報のみ／日付付き告知は本人が手動投稿）----
  sec_(form, 'セクション5：イベント（背景情報）');
  par_(form, '定期的にやっているイベント・セミナーの種類と頻度', '例：毎月の交流会／無料セミナー／個別相談会。自動投稿が日付なしで「毎月交流会をやっています」と自然に触れるために使います。なければ「特になし」', true);
  par_(form, 'イベントに来た人にどうなってほしいか', 'イベントの魅力を自動投稿でにおわせるために使います', true);
  par_(form, '（補足）日付が決まった具体的なイベント告知について', '日付が入る告知は、鮮度が大事なので、ナポリさんご自身でThreadsに手動投稿していただく形です（自動投稿は普段の思い・信念を担当します）。ここは確認だけなので「了解」と書いて次へ進んでください', false);
