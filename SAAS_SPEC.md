...
4. **決済完了と同時に、サロン情報のアンケートフォームがLINEで自動送信される**ので回答（`stripe_webhook.py`が送信・彩さんの手動送付は不要／LINE ID不明時のみ手動）。文面は「**詳しく書くほど投稿の質が上がる／あっさりだとありきたりになる**」と明記して丁寧な回答を促す
...
- [x] フォームは決済完了で**自動送信**される（手動送付は不要／LINE ID不明時のみ手動）
...
| 決済完了 → フォームのLINE自動送信 | **即時（自動・`stripe_webhook.py`）** |
| フォーム回答 → アカウント連携作業 | **回答確認後2営業日以内** |
| 連携完了 → 投稿開始 | 当日（手動実行）または翌朝9時自動投稿 |

> クライアントへの案内文（ウェルカムシナリオ2通目に含める）：
> 「ご決済後すぐに、サロン情報のアンケートフォームをLINEで自動送信します。**詳しくご回答いただくほど投稿の質が上がります**。ご回答後、担当者が設定（約2分）を行い、当日〜翌日から自動投稿が開始されます。」
