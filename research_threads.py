...
- 1ルールは50文字以内
- 【重要】企業・機器メーカー・B2Bサービス販売アカウント（施設向け機器・システム販売・業務ツール等）のパターンは除外すること。個人発信者・サロンオーナー・一般ユーザーの投稿パターンのみを参考にすること
- 【重要】「施設」「実装」「導入」などB2B・IT系の用語がルールに含まれる場合は、美容サロン向けの自然な表現（「施術後」「来店後」等）に置き換えること
- 【重要】英単語のスペルは必ず正確に書くこと（例：Before / After を Befoer 等と誤記しない）
"""
