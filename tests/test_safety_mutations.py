...
MUTATIONS = [
    ...
    ("埋める直前の再確認",
     ...
     "                if False:\n                    continue"),
    ("公開の痕跡が残る台帳から出し直さない（入口）",
     "    if any(_has_history(p) for p in parts):",
     "    if any(p.get(\"status\") in (PART_CONTAINER, PART_UNKNOWN, PART_PUBLISHED)\n"
     "           for p in parts):", TARGET_STATE),
    ("失敗があれば点検済みにしない",
     "    if not failed:",
     "    if True:"),
    ...
    ("リンクの中身は本文として読まない",
     ...
     '    text = str(text or "")', TARGET_BOTLIB),
