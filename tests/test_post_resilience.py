...
_LASTCALL = {"サロン名": "ファミリエ", "価格を投稿に記載してもOKですか？": "いいえ（詳しくはDMまたはHPへ誘導する）",
             "提供メニューと価格帯（箇条書きでOK）": "",
             "一番の売りメニュー・最も結果が出やすい施術": "",
             "所在地（最寄り駅・徒歩時間）": "JR吹田駅 徒歩13分",
...
_FACT_CASES = [
    ...
    ("営業時間の中の時刻は通す", _MAN2, "12時に来店したい方へ。当店はその時間も営業しています。", True),
    ("営業時間の外の時刻は止める", _MAN2, "20時まで営業しています。", False),
    ("閉店時刻の言い切りは完全一致を求める", _MAN2, "17:59まで営業しています。", False),
    ("正しい閉店時刻は通す", _MAN2, "18:00まで営業しています。", True),
    ("正しい開店時刻は通す", _MAN2, "9時から営業しています。", True),
    ("違う開店時刻は止める", _MAN2, "10時から営業しています。", False),
    ("最終受付が併記された営業時間でも閉店時刻を取り違えない", _LASTCALL,
     "9:30〜17:30で承っています。", True),
    ("最終受付付きでも営業時間外は止める", _LASTCALL, "19時まで営業しています。", False),
    ("違う最終受付は止める", _LASTCALL, "最終受付は17時です。", False),
    ("正しい最終受付は通す", _LASTCALL, "最終受付は16:30です。", True),
    ("最終受付の記載が無い店では言い切らせない", _MAN2, "最終受付は17時です。", False),
    # 2026-09-13 Sol 3巡目の反例
    ("「◯時まで承っています」も閉店の言い切り", _LASTCALL, "当店は22時まで承っています。", False),
    ("営業時間そのものの範囲表記を照合する", _LASTCALL, "営業時間は12:00〜16:00です。", False),
    ("正しい範囲表記は通す", _LASTCALL, "営業時間は9:30〜17:30です。", True),
    ("範囲の前後が逆なら止める", _LASTCALL, "営業時間は17:30〜9:30です。", False),
    ("「◯時が最終受付です」も照合する", _LASTCALL, "17時が最終受付です。", False),
    ("開店と閉店を同じ文に書いても取り違えない", _LASTCALL,
     "9時30分に開店し、17時30分に閉店します。", True),
    ("「区別」を地名と誤判定しない", _MAN2, "ケアを区別して考えてください。", True),
    ("「1万人」を金額と誤判定しない", _MAN2, "1万人以上の方を見てきました。", True),
    ("「3千人」を金額と誤判定しない", _MAN2, "3千人の方が受けています。", True),
    # 2026-09-13 実害：予約URLの中の数字を番地と誤認し、判断材料が15本すべて落ちた
    ("予約URLの中の数字を住所と誤判定しない", _MAN2,
     "ご予約はこちらから。\nhttps://s.lmes.jp/landing-qr/2008022680-0gNljz7e?uLand=Gv9UOT", True),
    ("lit.linkのUUIDも誤判定しない", _MAN2,
     "ご予約はこちら\nhttps://lit.link/82d8a461-86fa-42ab-a1fb-a5a958a9c73b", True),
    ("URLの中の金額らしき数字も拾わない", _MAN2,
     "詳しくはこちら https://example.jp/plan?price=7777円", True),
    ("本文（URLの外）の架空の番地は止める", _MAN2,
     "渋谷区1丁目99-99にあります。ご予約は https://lin.ee/abc から。", False),
...
_TREE_CASES = [
    ("部をまたいだ営業時間の嘘を止める", _MAN2,
     ["当店の営業時間。最後に受け付けるのは", "夜22時です。"], False),
    ("部をまたいでも正しければ通す", _MAN2,
     ["当店の営業時間。最後に受け付けるのは", "17時です。"], True),
    ("部をまたいだ金額の嘘を止める", _MAN2,
     ["初回のご案内です。お値段は", "7,700円です。"], False),
    ("生活の時刻を営業時間と誤判定しない", _MAN2,
     ["朝の光が気持ちいい季節です。", "初めての方はお肌を見せていただきます。"], True),
    ("別の文の生活の時刻は落とさない", _MAN2,
     ["朝7時に家を出て、仕事へ向かう毎日。", "当店は9時から営業しています。"], True),
    ("文をまたいだ最終受付の嘘を止める", _LASTCALL,
     ["当店の最終受付についてご案内します。", "夜22時です。ご予約はお早めに。"], False),
    ("1部目だけの嘘も止める", _MAN2,
     ["つくば市にあります。", "初めての方はお肌を見せていただきます。"], False),
    ("2部目だけの嘘も止める", _MAN2,
     ["初めての方へ。まずお肌を見せてください。", "新宿駅から3分です。"], False),
]
_bad2 = [(nm, judge_fact_violations(t, sal))
         for nm, sal, t, ok in _TREE_CASES
         if (judge_fact_violations(t, sal) is None) != ok]
check("ツリーの事実照合が全件正しい", not _bad2, _bad2)
