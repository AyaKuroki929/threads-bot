...
- 350〜450字（必ず500字未満）
- 段落を空行で分ける
- 締めは「（誰への声かけか一言）＋公式LINEからお問い合わせください。」を基本形にし、最後の行は必ず {LINE_URL}
  - 例：「止まったままのアカウントがある方は、公式LINEからお問い合わせください。」「詳細は、公式LINEからお問い合わせください。」
  - この基本形は彩さん指定（2026-07-28）。「中身はこちらに置いてあります」のような場所の案内だけの誘導は禁止（読者が動く理由にならない）
  - 「〜より」でなく「〜から」（書類語を混ぜない）。頭の呼びかけ部分は毎回変える
